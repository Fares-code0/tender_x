...
| **طور التقوية (H0–H8)** | ✅ مكتمل ومُتحقَّق بالكامل — `v2.0.0` ثم `v2.1.0` | 2026-07-31 | يُتابَع في `CLAUDE_CODE_PROMPT_Tender_Hardening.md` (H7.1 أُغلق: صورة مبنيّة + 18/18 فحص دخان) |
...
| 2026-07-21 | M3 | `db:seed` يُشغَّل كسكربت مستقل خارج التطبيق فلا يقرأ `.env` ⇒ يفشل بلا `DATABASE_URL` | تمرير الملف صراحةً في السكربت: `tsx --env-file-if-exists=.env prisma/seed.ts` |
| 2026-07-21 | M3 | كل تغيير حالة يتطلب **ثلاث** كتابات مترابطة (الحالة + `TenderStatusHistory` + قيد Audit)؛ تكرارها في كل مسار يفتح باب الانحراف والكتابة الجزئية ويكسر BR-008 | `lib/statusChange.ts::recordStatusChange` — تنفّذ الثلاثة **داخل معاملة واحدة**، وصارت المعبر الوحيد لتغيير الحالة (ثم غلّفتها الـState Machine في M4) |
| 2026-07-22 | M4 | مسارا M3 (`review/start` و`review/decision`) كانا يغيّران الحالة مباشرة؛ بإنشاء الـState Machine صار للنظام **مصدرا حقيقة** لقواعد الانتقال | إعادة توجيه مسارَي M3 عبر `resolveTransition` أيضًا — جدول `TRANSITIONS` هو المرجع الوحيد، والاختبار يمرّ على كل صفوفه برمجيًا فلا يتقادم |
| 2026-07-22 | M5 | `multer` يفكّ ترميز `originalname` كـlatin1 ⇒ أسماء الملفات العربية تُخزَّن مشوّهة | إعادة تفسير البايتات: `Buffer.from(name,'latin1').toString('utf8')` في `routes/tenders/attachments.ts:29` + اختبار «يحافظ على اسم ملف عربي» |
| 2026-07-22 | M6 | مهمة الـcron تعتمد على «الآن»، فاختبارها يتطلب التلاعب بساعة النظام (هشّ وبطيء) | جعل الوقت **مَحقونًا**: `runClosingReminders(now = new Date())` — الاختبار يمرّر تاريخًا صريحًا، ويؤكد أن تشغيلين متتاليين يُنتجان إشعارًا واحدًا فقط |
| 2026-07-22 | M7 | `res.clearCookie` استُدعي بخيارات **لا تطابق** خيارات الضبط، فلم يمسح المتصفح كوكي الجلسة فعليًا و`/me` بقي يعيد 200 بعد الخروج | استخراج `cookieOptions` واحدة مشتركة بين الضبط والمسح (`routes/auth.ts:18`) مع تعليق يحذّر من فكّها؛ التحقق: `/me` → 401 بعد الخروج |
| 2026-07-22 | M7 | تصدير CSV يظهر بعربية مشوّهة عند فتحه في Excel (لا يفترض UTF-8) | إضافة BOM في مقدّمة الملف (`ReportsPage.tsx:27`) — تُقرأ العربية سليمة، والأرقام طُوبقت مع استعلام مباشر على القاعدة |
| 2026-07-22 | M8 | تفعيل محدِّد المعدل على `/auth/login` يكسر بقية المجموعة: عشرات الاختبارات تسجّل الدخول فتتجاوز الحد وتعيد 429 | تعطيله في بيئة `test` افتراضيًا (`app.ts:43`) وتفعيله **صراحةً** فقط في اختباره عبر `createApp({ rateLimit: true })` — الضابط يُختبَر بلا أن يلوّث غيره |
| 2026-07-22 | ما بعد v1.0 | `AdminUsersPage` كانت للقراءة فقط رغم أن API إدارة المستخدمين مُنفَّذ ومُختبَر منذ M1.5 — فجوة واجهة/API | ربط الواجهة بالـAPI: مودال إضافة + تغيير دور + تعطيل/تفعيل، مع **حارس ذاتي** يمنع الأدمن من تغيير دوره أو تعطيل حسابه (`ca35e21`) |
| 2026-07-22 | ما بعد v1.0 | فجوات SCR-03: الـAPI يدعم `assigneeId`/`closingBefore`/`closingAfter` لكن الواجهة لا تعرضها، والترتيب غير مدعوم أصلًا | إضافة `sort` للـAPI والواجهة + قائمة المسؤولين عبر `GET /users` **بحقول محدودة (id/name/role) لا تكشف البريد** (`98cc6f1`) |
| 2026-07-22 | ما بعد v1.0 | BR-009 يشترط أن تكون مهلة التذكير قابلة للتعديل، لكنها كانت في `SystemSetting` بلا endpoint ولا واجهة ⇒ القاعدة غير محقَّقة عمليًا | `GET/PATCH /admin/settings` (ADMIN) + صفحة إعدادات، والـcron يقرأ القيمة **حيًّا** عبر `getReminderDays()` فتسري على التشغيل التالي (`429a1e8`) |
| 2026-07-23 | ما بعد v1.0 | فجوة ACT-02: `PATCH /tenders/:id` كان يسمح بالتعديل في **أي** حالة، بما فيها مناقصة مُقدَّمة أو مغلقة | `LOCKED_TENDER_STATUSES` + `isTenderEditable()` في `shared`؛ الخادم يرفض بـ422 `TENDER_LOCKED` والواجهة تُخفي رابط التعديل (`87d9fc7`) |
| 2026-07-26 | M3–M8 (process) | **هذا السجل نفسه توقّف عند M2**، وكذلك قوائم التحقق ولوحة التقدم — فبدا الملف وكأن نصف المنتج لم يُبنَ، وهي نفس الحالة التي أضلّت جلسة سابقة في طور التقوية فأعادت تنفيذ H0 بالكامل | مزامنة الملف مع الواقع وإعادة بناء صفوف M3–M8 أدناه **من الأدلة** (رسائل الالتزامات، وتعليقات الكود، والاختبارات) — راجع ملاحظة المصدر تحت الجدول |

> **📌 مصدر صفوف M3 فما بعد:** الصفوف حتى M2 كُتبت **أثناء العمل** كما تنص القاعدة 5.
> أما صفوف M3–M8 وما بعد v1.0 فقد **أُعيد بناؤها لاحقًا (2026-07-26)** لأن السجل تُرك فارغًا وقتها،
> ومصدرها أدلة قابلة للتدقيق: متون التزامات المراحل، وتعليقات في الكود تشرح سبب الحل
> (مثل `routes/auth.ts:18` و`attachments.ts:28`)، والاختبارات التي تحرس كل إصلاح.
> **لذلك هي أقل اكتمالًا من الواقع:** ما لم يترك أثرًا في الكود أو في رسالة التزام لم يكن ممكنًا استرجاعه —
> المشاكل التي حُلّت داخل الجلسة بلا التزام مستقل ضاعت. سجّل المشكلة **لحظة حدوثها** لا لاحقًا.
> سجل طور التقوية (H0–H8) منفصل وكامل في `CLAUDE_CODE_PROMPT_Tender_Hardening.md`.
