...
> **تحديث M8 (سجل العمليات + التقوية) — v1.0:** BR-008 (تسجيل كل إجراء جوهري في Audit) مدعومة بواجهة عرض للقراءة فقط عبر `GET /tenders/:id/audit` (ACT-16، تبويب "سجل العمليات" في SCR-04) — و**NFR-005**: لا يوجد أي endpoint حذف أو تعديل للـAudit في أي راوتر. كما أُضيفت تقوية أمنية: Helmet، CORS مضبوط على أصل الواجهة، وتحديد معدل تسجيل الدخول (5 محاولات/15 دقيقة → 429)، مع معالجة أخطاء موحّدة `{error:{code,message}}`.

> **تحديثات ما بعد v1.0:** BR-009 صارت **قابلة للتعديل من الواجهة** — شاشة إعدادات (SCR-09، ADMIN) عبر `GET`/`PATCH /admin/settings` تضبط `closingReminderDays` (ACT-17). وأُغلقت فجوة ACT-02: بيانات المناقصة تُقفَل بعد `SUBMITTED`/`WON`/`LOST`/`REJECTED` (`isTenderEditable` → 422 `TENDER_LOCKED`).
