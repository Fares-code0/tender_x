...
|---|---|
| SCR-01 | تسجيل الدخول |
| SCR-02 | لوحة المعلومات (Dashboard — كروت + رسوم Recharts حسب الدور) |
| SCR-03 | قائمة المناقصات |
| SCR-04 | تفاصيل المناقصة (تبويبات التفاصيل/المراجعة/المرفقات + شريط الإجراءات + جرس الإشعارات في الهيدر) |
| SCR-05 | نموذج المناقصة (إنشاء/تعديل) |
| SCR-06 | إدارة المستخدمين (Admin) |
| SCR-07 | مهامي (My Tasks) |
| SCR-08 | التقارير (Reports — Manager/Owner/Admin + تصدير CSV) |
| SCR-09 | إعدادات النظام (Settings — Admin: أيام تنبيه الإغلاق) |

> **تحديث M3 + M4 (حالة التنفيذ):** جميع رحلات دورة الحياة أدناه (JRN-02 حتى JRN-06) صارت **منفّذة فعليًا** في الكود عبر انتقالات الـState Machine المركزية (`services/tenderWorkflow.ts`). تُنفَّذ إجراءات المراجعة والـChecklist داخل تبويب "المراجعة" في SCR-04، وتظهر أزرار انتقالات سير العمل ديناميكيًا في **شريط الإجراءات (ActionsBar)** أعلى SCR-04 حسب (الحالة + الدور). أُضيفت شاشة **SCR-07 "مهامي"** التي تعرض للمستخدم مناقصاته حسب دوره (QA/WRITER: المعيّنة له؛ MANAGER: بانتظار اعتماده).

> **تحديث M7 (لوحات المعلومات والتقارير):** أُضيفت لوحة معلومات حسب الدور (SCR-02، `GET /dashboard`) كنقطة انطلاق بعد الدخول لكل الرحلات، وصفحة تقارير (SCR-08، `GET /reports/summary` + تصدير CSV) تخدم دور المتابعة الإشرافي في JRN-07 (المالك) والمدير — كلاهما قراءة/تصدير فقط دون انتقالات حالة (ACT-15).

> **تحديث M8 (سجل العمليات + التقوية) — v1.0:** يخدم JRN-07 (والمدير) أيضًا تبويب "سجل العمليات" في SCR-04 (`GET /tenders/:id/audit`، ACT-16) لمتابعة من نفّذ ماذا ومتى على المناقصة — قراءة فقط (NFR-005). المشروع مكتمل حتى الإصدار v1.0.

> **تحديث M5 + M6 (حالة التنفيذ):** خطوة "رفع/إدارة المرفقات (ACT-13)" في JRN-03 صارت **منفّذة** — تبويب "المرفقات" في SCR-04 (رفع بـdrag & drop، قائمة، تحميل) خلف `StorageService` مع قيود النوع/الحجم وversioning. وأُضيف **نظام الإشعارات (M6)** كطبقة عرضية على كل الرحلات: كل حدث سير عمل (إنشاء→QA، تعيين→الكاتب، إرسال للاعتماد→المدراء، إعادة→الكاتب، اعتماد→الكاتب المُرسِل، نتيجة→المنشئ) يولّد إشعارًا يظهر في **جرس الإشعارات** بالهيدر ويوصّل صاحبه إلى صفحة المناقصة (SCR-04)؛ إضافةً إلى تنبيه اقتراب الإغلاق (BR-009). لم تبقَ خطوة دورة حياة مؤجَّلة.
...
| رفع/إدارة المرفقات (تبويب "المرفقات" في SCR-04، منفّذ M5) | WRITER | PROPOSAL_PREPARATION | PROPOSAL_PREPARATION | ACT-13 | SCR-04 | ملف بصيغة غير مدعومة (422) أو يتجاوز 20MB (413) |
